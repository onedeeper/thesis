...
from eeglearn.preprocess.clean import (
...
class TestClean:
    ...
    def setup_clean_pipeline_test(self):
        ...
        self.derivatives_dir = os.environ.get('EEG_TEST_DERIVATIVES_DIR',
                                              self.default_derivatives_dir)
        
        # Skip test if environment variable is not set
        if self.derivatives_dir == self.default_derivatives_dir:
            pytest.\
                skip("EEG_TEST_DERIVATIVES_DIR env var not found. Skipping test.")
    ...
    def test_process_file(self, setup_process_file_test):
        ...
        expected_file = expected_dir / f'{self.ID}_{self.sessID\
                                                    }_{self.cond}_preprocessed.npy'
        ...
        for subj_id in ['sub-001', 'sub-002', 'sub-003']:
            ...
            for session in ['ses-1', 'ses-2']:
                ...
                for condition in ['EO', 'EC']:
                    ...
                    file_path = os.path.join(sess_dir, 
                                             f'{subj_id}_{session\
                                                          }_task-rest{condition\
                                                                      }_eeg.csv')
    ...
    def test_get_file_details_output_structure(self, setup_get_file_details_test):
        ...
        # Check that all returned items are Preprocessing_params 
        # instances with correct fields
        ...
        expected_path = os.path.join(
            ...
            details.session, 
            'eeg', 
            f'{details.participant}_{details.session}_task-rest{details.\
                                                                condition}_eeg.csv'
        )
    ...
    def test_get_file_details_missing_files(self, setup_get_file_details_test):
        ...
        with patch('os.path.exists') as mock_exists:
            ...
            assert os.path.exists('preprocessing_log.txt'), \
                "Log file not created for missing files"
            
            # Verify missing subject is not included
            subjects = [details.participant for details in file_details]
            assert missing_subject not in subjects, \
                f"Missing subject was incorrectly included: {subjects}"
    ...
    def test_get_file_details_exception_handling(self, setup_get_file_details_test):
        ...
        with patch('os.path.exists') as mock_exists:
            ...
            assert len(file_details) == 4, \
                f"Expected 4 files after exceptions, got {len(file_details)}"
            
            # Verify no files for the subject that raised the exception
            subjects = set(details.participant for details in file_details)
            assert 'sub-002' not in subjects, \
                f"Subject with exceptions was incorrectly included: {subjects}"
