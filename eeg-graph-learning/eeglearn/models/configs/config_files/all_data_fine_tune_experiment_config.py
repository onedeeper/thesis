...
class Config:
    ...
    lr = 0.0016112816781597217 # <----  classification head
    weight_decay = 1.7714226782542088e-06 # <---- classification head
    drop_rate = 0.283564938787352 # <--- classification head
    stop_at = 10
    ...
    linear_size = 1024 # <--- Linear layers
    n_eeg_channels = 26

    # Parameters for pre-trained GCN feature extractor
    pretrained_weights_path = "/mnt/disk2/thesis/eeg-graph-learning/data/weights/no_cv/self_supervised/all_data/fine_tune_all_data_best_model_val_loss_1.1467_epoch_30.pt"
    pretrained_gcn_out_size = 128  # Example: Must match your actual SSL model
    pretrained_k = 1              # Example: Must match your actual SSL model
    pretrained_linear_size = 1024  # Example: Must match your actual SSL model's HF/HS linear_size
    pretrained_drop_rate = 0.302707790958443    # Example: Must match your actual SSL model's HF/HS drop_rate
