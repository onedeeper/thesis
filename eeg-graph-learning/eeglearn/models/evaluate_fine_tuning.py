...
from eeglearn.utils.models import (setup_label_encoder, create_graph_loaders, 
                                   calculate_class_weights, split_data, bootstrap_ci_f1) 
...
def evaluate():
    ...
    model = JointlyTrainModel(
        inchannel=5,
        gcn_out_size=Config.pretrained_gcn_out_size,
        batch=batch_size,
        K=Config.pretrained_k,
        linear_size=Config.pretrained_linear_size,
        drop_rate=Config.pretrained_drop_rate,
        linear_size_hc=Config.linear_size,
        drop_rate_hc=Config.drop_rate,
        testmode=False,
        HF=120, HS=128, HC=n_classes
    ).to(device)

    # Load model weights
    model_path = Config.pretrained_weights_path
    ...
    # Load test data
    test_graphs_path = "/mnt/disk2/thesis/eeg-graph-learning/data/weights/best_models/tuur_data/tuur_data_test_original_graph_list.pt"
